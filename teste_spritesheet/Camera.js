...
var Camera = Class.extend(function () {
	...
	this.center = function (gameWorld) {
		this.x = (gameWorld.x + gameWorld.sprite.sourceHeight / 2) - this.width / 2;
		this.y = (gameWorld.y + gameWorld.sprite.sourceHeight / 2) - this.height / 2;
	}

	this.keepWithinWorld = function (gameWorld, trackingEntity) {
		camera.x = Math.max(0, Math.min(Math.floor(trackingEntity.x + (trackingEntity.width / 2) - (this.width / 2)), gameWorld.sprite.sourceWidth - this.width));
		camera.y = Math.max(0, Math.min(Math.floor(trackingEntity.y + (trackingEntity.height / 2) - (this.height / 2)), gameWorld.sprite.sourceHeight - this.height));
	}
